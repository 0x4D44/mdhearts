...
fn build_round(
    ...
    // seed with a complete trick to ensure this is not first trick
    for card in [
...
#[test]
fn endgame_feed_cap_does_not_break_choice() {
    // Scenario similar to endgame_golden: feed QS to leaderboard near 100
    ...
    let scores = build_scores([40, 95, 45, 60]);
    ...
    // Without cap (default 0), expect QS choice
    let choice_no_cap = PlayPlanner::choose(&legal, &ctx).unwrap();
    assert_eq!(choice_no_cap, Card::new(Rank::Queen, Suit::Spades));

    // With a very low cap, the preference remains (functional check; not asserting score equality)
    unsafe { std::env::set_var("MDH_W_ENDGAME_FEED_CAP", "50") };
