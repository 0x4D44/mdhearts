...
#[test]
fn endgame_feed_hearts_to_leader_when_void() {
    // East (leader) near 100 leads clubs; we (South) are void in clubs and can dump a heart.
    // Expect: choose to dump a heart (non-QS) to feed the leader.
    ...
    let scores = build_scores([40, 95, 45, 60]); // East leads the scoreboard near 100
