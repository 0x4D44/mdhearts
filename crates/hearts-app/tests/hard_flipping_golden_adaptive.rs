...
fn west_seed_1141_expected_tops_under_adaptive_and_sampling() {
    ...
    assert_eq!(n_top, Card::new(Rank::Three, Suit::Spades), "Normal top changed; update golden if intentional");
    let expected = [Card::new(Rank::Five, Suit::Diamonds), Card::new(Rank::Ace, Suit::Diamonds)];
    assert!(expected.contains(&h_top), "Hard top changed; got {:?}; update golden if intentional", h_top);
}
...
fn west_seed_1082_expected_tops_under_adaptive_and_sampling() {
    ...
    assert_eq!(n_top, Card::new(Rank::Ten, Suit::Spades), "Normal top changed; update golden if intentional");
    let expected = [Card::new(Rank::Jack, Suit::Diamonds), Card::new(Rank::Two, Suit::Spades)];
    assert!(expected.contains(&h_top), "Hard top changed; got {:?}; update golden if intentional", h_top);
}
