...
- 2025-10-30: When leading and the Stage 2 guards apply, we now simulate the rest of the trick (`simulate_lead_outcome`) and prefer any candidate that actually hands off the trick; otherwise we minimize projected penalties using the simulated outcome. Hits triggered inside the probe no longer leak into the real planner stats. A new unit test (`lead_simulation_avoids_penalty_when_followers_duck`) codifies the constructed seat where opponents duck high cards.
- 2025-10-30: DP flip golden tests now reference seeds `(1061 West, 1052 South, 3383 East)` that still flip under default weights; the flip-from-seed helpers keep additional search utilities under `#[ignore]` for future seed-hunting.
